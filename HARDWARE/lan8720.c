...
u8 LAN8720_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOG , ENABLE);//使能GPIO时钟 RMII接口
	...
	GPIO_ResetBits(GPIOB,GPIO_Pin_15);					//硬件复位LAN8720
	delay_ms(100);	
	GPIO_SetBits(GPIOB,GPIO_Pin_15);				 	//复位结束 
...
/** get link status, 0 normal, 1 break */
u8 LAN8720_Get_link(void) {
	u8 link;
	link=((ETH_ReadPHYRegister(0x00,29)&0x10)>>4); //read LAN8720 register 29 to get link status
	return link;
} 
