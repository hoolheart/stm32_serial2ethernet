...
{
	u32 sn0;//获取STM32的唯一ID的前24位作为MAC地址后三字节
	//get configuration
	SYS_CONFIG *cfg = getConfig();
	sn0=*(vu32*)(0x1FFF7A10);//获取STM32的唯一ID的前24位作为MAC地址后三字节
	//默认远端IP
	lwipx->remoteip[0]=cfg->remote_ip[0];	
	lwipx->remoteip[1]=cfg->remote_ip[1];
	lwipx->remoteip[2]=cfg->remote_ip[2];
	lwipx->remoteip[3]=cfg->remote_ip[3];
	...
	//默认本地IP
	lwipx->ip[0]=cfg->ip_addr[0];	
	lwipx->ip[1]=cfg->ip_addr[1];
	lwipx->ip[2]=cfg->ip_addr[2];
	lwipx->ip[3]=cfg->ip_addr[3];
	//默认子网掩码
	lwipx->netmask[0]=cfg->ip_mask[0];	
	lwipx->netmask[1]=cfg->ip_mask[1];
	lwipx->netmask[2]=cfg->ip_mask[2];
	lwipx->netmask[3]=cfg->ip_mask[3];
	//默认网关
	lwipx->gateway[0]=cfg->ip_gate[0];	
	lwipx->gateway[1]=cfg->ip_gate[1];
	lwipx->gateway[2]=cfg->ip_gate[2];
	lwipx->gateway[3]=cfg->ip_gate[3];	
